...
When GitHub updates are enabled in Serpulix SEO settings, the plugin also contacts GitHub
to look for a newer release.

* What it sends/receives: the site requests the latest GitHub Release for the repository
  you configure. If you save a personal access token for a private repository, that token
  is sent only from the server to api.github.com and is not shown in the browser. The
  plugin receives the release version, notes, and update ZIP.
* When: when WordPress checks for plugin updates (about twice a day, or when you click
  Check Again), and when you install an update.
* Service: GitHub — https://github.com (Terms:
  https://docs.github.com/en/site-policy/github-terms/github-terms-of-service, Privacy:
...
* GitHub releases can supply plugin updates from Serpulix SEO → Settings when GitHub
  updates are enabled.
